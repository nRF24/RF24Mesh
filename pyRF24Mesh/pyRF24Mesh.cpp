...
BOOST_PYTHON_MODULE(RF24Mesh){
    { //::RF24Mesh
        ...
        .def("begin", RF24Mesh::begin, begin_overload(args("channel", "data_rate", "timeout")))
        //uint8_t update();
        .def("update", &RF24Mesh::update)
        //bool write(const void* data, uint8_t msg_type, size_t size, uint8_t nodeID=0);
        .def("write", &RF24Mesh::write, write_overload(args("data", "msg_type", "size", "nodeID")))
        ...
        .def("getNodeID", &RF24Mesh::getNodeID, getNodeID_overload(args("address")))
        //bool checkConnection();
        .def("checkConnection", &RF24Mesh::checkConnection)
        //uint16_t renewAddress(uint32_t timeout=MESH_RENEWAL_TIMEOUT);
        .def("renewAddress", &RF24Mesh::renewAddress, getNodeID_overload(args("timeout")))
        //bool releaseAddress();
